...
class Shapelet:
    ...
    def make_temp_catalogs(self,inimage,catpath,outpath,imgtype):
        ...
        self.sfiles = '/export/gotodata1/kackley/github/zernike/zernike/param/goto.sex'
        ...
        try:
            ...
            if argtest:
                self.fname = os.path.basename(self.inimage)
                cat_file = self.fname.replace('.fits','_in.cat')
                fullsextrcat = os.path.join(self.catpath,cat_file)
                fits.getdata(fitsfile,header=False)
                fileIO.sextractor_script(fitsfile, fullsextrcat, self.sfiles)
                cat_data = fits.getdata(fitsfile,header=False)
                return fullsextrcat, cat_data
            else:
